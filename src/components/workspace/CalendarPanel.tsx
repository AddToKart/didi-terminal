...
export function CalendarPanel({ workspaceId, isOpen, onClose }: CalendarPanelProps) {
  ...
  return (
    <div className="fixed inset-4 top-[48px] bg-[#08080a] border border-zinc-800 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.9)] z-[60] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 pointer-events-auto">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-[#0d0d10] shrink-0">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={today}
              className="px-3 py-1.5 text-xs font-semibold text-zinc-300 hover:text-white bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-md transition-colors"
            >
              Today
            </button>
            <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-md overflow-hidden">
              <button onClick={prevMonth} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors">
                <ChevronLeft size={16} />
              </button>
              <div className="w-px h-4 bg-zinc-800" />
              <button onClick={nextMonth} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors">
      ...
      {/* Calendar Grid */}
      <div className="flex flex-col flex-1 min-h-0 bg-[#030304]">
        {/* Days of Week */}
        <div className="grid grid-cols-7 border-b border-zinc-800 shrink-0 bg-[#0a0a0d]">
          ...
          {daysInMonth.map((day, i) => {
            ...
            return (
              <div 
                ...
                className={`
                  group border-r border-b border-zinc-800 p-2 flex flex-col gap-1 overflow-hidden transition-colors cursor-pointer
                  ${!isCurrentMonth ? 'bg-[#040406]/90 opacity-40' : 'hover:bg-zinc-900/40'}
                  ${i % 7 === 6 ? 'border-r-0' : ''}
                `}
              >
