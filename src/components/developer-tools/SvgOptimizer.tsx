  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-0 sm:p-3">
        <div className="bg-[#0b0b0d]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_60px_Rgba(0,0,0,0.6)] overflow-hidden flex flex-col w-full max-w-4xl h-[80vh] sm:rounded-xl">
          ...
          <div className="flex-1 flex flex-col sm:flex-row min-h-0">
            {/* Input */}
            <div className="flex-1 flex flex-col border-r-0 sm:border-r border-white/5 min-h-0">
              <div className="px-4 py-2 text-[9px] text-zinc-600 uppercase tracking-wider font-bold bg-black/20 shrink-0 flex items-center justify-between">
                <span>Input SVG</span>
                <span className="font-mono">{inputSize} B</span>
              </div>
              <textarea value={input} onChange={e => setInput(e.target.value)}
                placeholder="Paste SVG code here..."
                className="flex-1 bg-black/20 p-4 text-xs font-mono text-zinc-400 resize-none outline-none border-none min-h-[120px] sm:min-h-0"
                spellCheck={false} />
            </div>

            {/* Output */}
            <div className="flex-1 flex flex-col min-h-0">
              <div className="px-4 py-2 bg-black/20 text-[9px] uppercase tracking-wider font-bold shrink-0 flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-3">
              ...
              {/* Preview */}
              {previewSvg && (
                <div className="h-48 sm:h-56 shrink-0 bg-white/[0.02] border-b border-white/5 flex items-center justify-center p-4" ref={previewRef}>
