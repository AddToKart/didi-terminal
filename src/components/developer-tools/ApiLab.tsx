  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[101] p-4 pointer-events-none">
        <div className="bg-[#0b0b0d]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden w-full max-w-5xl h-[750px] flex flex-col pointer-events-auto animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
          ...
          <div className="p-4 border-b border-white/5 bg-zinc-900/40 shrink-0">
            ...
            <div className="flex gap-2">
              <Select value={method} onValueChange={setMethod}>
                <SelectTrigger className="w-[120px] bg-black/40 border-white/10 text-xs font-bold font-mono shadow-inner h-10">
                  <SelectValue placeholder="Method" />
                </SelectTrigger>
                <SelectContent className="bg-zinc-950 border-white/10 text-white z-[200]">
                  {["GET", "POST", "PUT", "PATCH", "DELETE"].map(m => (
                    <SelectItem key={m} value={m} className="text-xs font-mono font-bold focus:bg-white/10 focus:text-white cursor-pointer">
                      {m}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <div className="relative flex-1">
                <input 
                  ...
                  className="w-full h-10 bg-black/40 border border-white/10 rounded-md pl-4 pr-10 text-xs font-mono text-white placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all shadow-inner"
                />
              </div>
          ...
          <div className="flex flex-1 min-h-0 divide-x divide-white/5">
            ...
            <div className="flex-[0.5] flex flex-col min-w-0 bg-black/20">
              <Tabs defaultValue="headers" className="flex-1 flex flex-col min-h-0">
                <div className="px-4 py-2 border-b border-white/5 shrink-0 bg-zinc-900/20">
                  <TabsList className="bg-black/40 border border-white/5 p-1 h-8">
                    <TabsTrigger value="headers" className="text-[10px] uppercase font-bold tracking-wider data-[state=active]:bg-white/10 data-[state=active]:text-white">Headers ({headers.filter(h => h.key).length})</TabsTrigger>
                    <TabsTrigger value="body" className="text-[10px] uppercase font-bold tracking-wider data-[state=active]:bg-white/10 data-[state=active]:text-white">Body</TabsTrigger>
                  </TabsList>
                </div>
                ...
                <TabsContent value="headers" className="flex-1 overflow-y-auto p-4 m-0 custom-scrollbar outline-none data-[state=inactive]:hidden">
                  <div className="space-y-2">
                    {headers.map((header, i) => (
                      <div key={i} className="flex gap-2 items-center group">
                        <input 
                          ...
                          className="flex-1 bg-black/40 border border-white/5 rounded-md px-3 py-1.5 text-xs font-mono text-blue-400 placeholder:text-zinc-700 outline-none focus:border-blue-500/50 transition-colors"
                        />
                        <input 
                          type="text"
                          placeholder="Value"
                          ...
                          className="flex-1 bg-black/40 border border-white/5 rounded-md px-3 py-1.5 text-xs font-mono text-zinc-300 placeholder:text-zinc-700 outline-none focus:border-zinc-500 transition-colors"
                        />
                        <button 
                    ...
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2 mt-2 border border-dashed border-white/10 hover:border-white/20 hover:bg-white/5 rounded-md text-[10px] font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-300 transition-all"
                    >
            ...
            <div className="flex-[0.5] flex flex-col min-w-0 bg-[#0a0a0c]">
              {error ? (
                ...
                <Tabs defaultValue="body" className="flex-1 flex flex-col min-h-0">
                  <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 shrink-0 bg-zinc-900/20">
                    <TabsList className="bg-black/40 border border-white/5 p-1 h-8">
                      <TabsTrigger value="body" className="text-[10px] uppercase font-bold tracking-wider data-[state=active]:bg-white/10 data-[state=active]:text-white">Body</TabsTrigger>
                      <TabsTrigger value="headers" className="text-[10px] uppercase font-bold tracking-wider data-[state=active]:bg-white/10 data-[state=active]:text-white">Headers</TabsTrigger>
                    </TabsList>
                  ...
                  <TabsContent value="headers" className="flex-1 overflow-auto m-0 outline-none data-[state=inactive]:hidden p-4">
                    <div className="space-y-1">
                      {Object.entries(response.headers).map(([k, v]) => (
                        <div key={k} className="flex gap-4 text-xs font-mono py-1 border-b border-white/5 last:border-0">
