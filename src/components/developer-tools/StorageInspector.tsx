  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-0 sm:p-3">
        <div className="bg-[#0b0b0d]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col w-full max-w-2xl h-[75vh] sm:rounded-xl">
          ...
          <div className="px-5 pt-5 pb-3 border-b border-white/5 bg-zinc-900/40 shrink-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={refresh} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-zinc-400 hover:text-white transition-all"><RefreshCw size={14} /></button>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex bg-black/30 rounded-lg p-0.5 border border-white/5">
                <button onClick={() => setTab("localStorage")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-bold transition-all ${tab === "localStorage" ? 'bg-blue-500/20 text-blue-400' : 'text-zinc-500 hover:text-zinc-300'}`}>
                  <HardDrive size={12} /> localStorage
                </button>
                <button onClick={() => setTab("cookies")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-bold transition-all ${tab === "cookies" ? 'bg-blue-500/20 text-blue-400' : 'text-zinc-500 hover:text-zinc-300'}`}>
              ...
              <div className="relative flex-1 min-w-[150px] group">
                ...
                <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
                  className="w-full bg-black/40 border border-white/5 rounded-lg py-1.5 pl-8 pr-3 text-xs text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/30" />
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-white/[0.03]">
            {tab === "localStorage" && (
              <div className="px-4 py-2 bg-black/20 border-b border-white/5 flex items-center gap-2 text-[10px]">
                <input placeholder="Key" value={newKey} onChange={e => setNewKey(e.target.value)}
                  className="flex-1 bg-black/40 border border-white/5 rounded px-2.5 py-1.5 text-xs text-zinc-300 placeholder:text-zinc-600 outline-none focus:border-emerald-500/30" />
                <input placeholder="Value" value={newValue} onChange={e => setNewValue(e.target.value)}
                  className="flex-[2] bg-black/40 border border-white/5 rounded px-2.5 py-1.5 text-xs text-zinc-300 placeholder:text-zinc-600 outline-none focus:border-emerald-500/30" />
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <div key={entry.key} className={`flex items-center gap-3 px-5 py-3 group hover:bg-white/[0.02] transition-colors ${isEditing ? 'bg-blue-500/5' : ''}`}>
                  <div className="flex-1 min-w-0">
                    ...
                    {isEditing ? (
                      <input value={editValue} onChange={e => setEditValue(e.target.value)}
                        ...
                        className="w-full bg-black/40 border border-blue-500/50 rounded px-2 py-0.5 text-[10px] font-mono text-zinc-300 outline-none mt-1" autoFocus />
                    ) : (
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => handleCopy(tab === "localStorage" ? `localStorage.getItem('${entry.key}')` : entry.value, entry.key)}
                      className="p-1.5 hover:bg-white/10 rounded text-zinc-500 hover:text-zinc-300 transition-all" title="Copy">
                      {isCopied ? <Check size={12} className="text-emerald-400" /> : <Copy size={12} />}
                    </button>
                    {tab === "localStorage" && (
                      <button onClick={() => handleEdit(entry.key)} className="p-1.5 hover:bg-white/10 rounded text-zinc-500 hover:text-zinc-300 transition-all" title="Edit">
                        <Edit3 size={12} />
                      </button>
                    )}
          ...
          <div className="px-5 py-2 border-t border-white/5 bg-white/[0.02] text-[9px] text-zinc-700 flex items-center justify-between shrink-0">
