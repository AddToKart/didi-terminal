...
export const BrainstormModal = ({ agents, sessions, onStart, onClose }: Props) => {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-app-panel/80 backdrop-blur-sm flex items-center justify-center p-8">
      <div className="w-full max-w-4xl max-h-full border border-zinc-800 bg-app-panel shadow-xl flex flex-col rounded-lg overflow-hidden relative">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
          ...
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-200 p-2 border border-zinc-800 hover:border-zinc-600 bg-app-panel rounded-sm shadow-sm transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6 border-r border-zinc-800">
            <div>
              ...
              <textarea
                ...
                className="mt-2 w-full h-32 bg-app-panel border border-zinc-800 focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20 rounded-md text-zinc-200 p-3 text-sm outline-none resize-none transition-all shadow-inner"
            ...
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-3">
                {agents.map(agent => (
                  <label key={agent} className="flex items-center gap-3 border border-zinc-800 bg-app-panel/50 hover:bg-zinc-900 px-3 py-2.5 rounded-md text-sm text-zinc-300 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={participants.includes(agent)}
                      onChange={() => toggleAgent(agent)}
                      className="accent-brand-accent w-4 h-4 rounded-sm border-zinc-700 bg-app-panel"
                    />
            ...
            <div className="flex items-center justify-between gap-4 pt-2">
              <label className="flex items-center gap-3 text-xs font-semibold text-zinc-400 tracking-tight">
                ...
                <input
                  ...
                  className="w-16 bg-app-panel border border-zinc-800 rounded-md text-zinc-200 px-3 py-1.5 text-sm outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20"
                />
              </label>
              <button
                type="submit"
          ...
          <div className="p-6 min-h-0 flex flex-col bg-zinc-900/10">
            ...
            {!activeSession ? (
              ...
              <div className="min-h-0 flex-1 flex flex-col">
                <div className="bg-app-panel border border-zinc-800 rounded-t-lg p-3">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-app-panel/50 border-x border-b border-zinc-800 rounded-b-lg shadow-inner">
          ...
          <div className="absolute inset-0 z-50 bg-app-panel/95 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
            <div className="w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-lg shadow-2xl flex flex-col max-h-full">
              ...
              {selectedAgentResponses.length > 1 && (
                <div className="px-5 py-3 border-t border-zinc-800 flex items-center justify-between bg-app-panel/50 rounded-b-lg">
                  <button 
