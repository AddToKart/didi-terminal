...
export const BrainstormModal = ({ agents, sessions, onStart, onClose }: Props) => {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-zinc-950/80 backdrop-blur-sm flex items-center justify-center p-8">
      <div className="w-full max-w-4xl max-h-full border border-zinc-800 bg-zinc-950 shadow-xl flex flex-col rounded-lg overflow-hidden relative">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
          ...
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-200 p-2 border border-zinc-800 hover:border-zinc-600 bg-zinc-950 rounded-sm shadow-sm transition-colors">
            <X size={18} />
          </button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] min-h-0 bg-app-bg">
          <form onSubmit={handleSubmit} className="p-6 space-y-6 border-r border-zinc-800">
            <div>
              <label className="text-xs font-semibold text-zinc-400 tracking-tight">Problem Statement</label>
              <textarea
                value={prompt}
                onChange={event => setPrompt(event.target.value)}
                className="mt-2 w-full h-32 bg-zinc-950 border border-zinc-800 focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20 rounded-md text-zinc-200 p-3 text-sm outline-none resize-none transition-all shadow-inner"
            ...
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-3">
                {agents.map(agent => (
                  <label key={agent} className="flex items-center gap-3 border border-zinc-800 bg-zinc-950/50 hover:bg-zinc-900 px-3 py-2.5 rounded-md text-sm text-zinc-300 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={participants.includes(agent)}
                      onChange={() => toggleAgent(agent)}
                      className="accent-brand-accent w-4 h-4 rounded-sm border-zinc-700 bg-zinc-950"
                    />
            ...
            <div className="flex items-center justify-between gap-4 pt-2">
              <label className="flex items-center gap-3 text-xs font-semibold text-zinc-400 tracking-tight">
                ...
                <input
                  ...
                  className="w-16 bg-zinc-950 border border-zinc-800 rounded-md text-zinc-200 px-3 py-1.5 text-sm outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20"
                />
              </label>
              <button
                type="submit"
                disabled={prompt.trim().length === 0 || participants.length < 2}
                className="bg-brand-accent hover:bg-blue-600 text-white disabled:bg-zinc-800 disabled:text-zinc-500 disabled:cursor-not-allowed px-5 py-2.5 rounded-md text-sm font-semibold transition-colors flex items-center gap-2 shadow-sm"
              >
                <RadioTower size={16} />
                Broadcast Debate
              </button>
          ...
          <div className="p-6 min-h-0 flex flex-col bg-zinc-900/10">
            ...
            {!activeSession ? (
              <div className="flex-1 border border-dashed border-zinc-800 rounded-lg flex items-center justify-center text-sm text-zinc-600 font-medium">
                No active debate
              </div>
            ) : (
              <div className="min-h-0 flex-1 flex flex-col">
                <div className="bg-zinc-950 border border-zinc-800 rounded-t-lg p-3">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-zinc-950/50 border-x border-b border-zinc-800 rounded-b-lg shadow-inner">
                    ...
                    Object.entries(agentResponses).map(([agentName, responses]) => {
                      ...
                      return (
                        <div 
                          ...
                          className="border border-zinc-800 bg-zinc-900 hover:bg-zinc-800/80 hover:border-zinc-600 p-3 rounded-md cursor-pointer transition-all shadow-sm group"
                        >
                          <div className="flex items-center justify-between gap-2 mb-2">
                            <span className="text-xs font-semibold text-brand-primary truncate">{agentName}</span>
                            <span className="text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded-sm font-medium">
        ...
        {selectedAgentId && currentCarouselResponse && (
          <div className="absolute inset-0 z-50 bg-zinc-950/95 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
            <div className="w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-lg shadow-2xl flex flex-col max-h-full">
              ...
              <div className="flex-1 overflow-y-auto p-6 text-sm text-zinc-300 leading-relaxed">
                <div className="mb-4 flex items-center gap-2">
                  <span className="text-xs font-bold text-zinc-500 bg-zinc-800 px-2 py-1 rounded-sm">Round {currentCarouselResponse.round}</span>
              ...
              {selectedAgentResponses.length > 1 && (
                <div className="px-5 py-3 border-t border-zinc-800 flex items-center justify-between bg-zinc-950/50 rounded-b-lg">
                  <button 
                    onClick={() => setCarouselRoundIndex(i => Math.max(0, i - 1))}
                    disabled={carouselRoundIndex === 0}
                    className="p-1.5 rounded bg-zinc-800 text-zinc-300 hover:bg-zinc-700 disabled:opacity-30 disabled:hover:bg-zinc-800 transition-colors flex items-center gap-1 text-xs font-medium"
                  >
