...
export function BrowserInstance({ id, url: initialUrl = "", onRemove, dragAttributes, dragListeners }: BrowserInstanceProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-app-panel text-zinc-300 font-sans border border-app-border rounded-lg overflow-hidden">
      {/* Header / URL Bar */}
      <div
        className="h-9 flex items-center gap-2 px-3 bg-app-bg border-b border-app-border shrink-0 select-none cursor-grab active:cursor-grabbing"
        {...dragAttributes}
        {...dragListeners}
      >
        ...
        <div className="flex items-center gap-1 px-2 border-l border-app-border ml-1 shrink-0">
          <button className="p-1 hover:bg-zinc-800/50 rounded transition-colors text-zinc-500 hover:text-zinc-300" title="Back">
            <ArrowLeft size={14} />
          </button>
          <button className="p-1 hover:bg-zinc-800/50 rounded transition-colors text-zinc-500 hover:text-zinc-300" title="Forward">
            <ArrowRight size={14} />
          </button>
          <button
            onClick={() => { if (url) openView(url); }}
            className="p-1 hover:bg-zinc-800/50 rounded transition-colors text-zinc-500 hover:text-zinc-300"
            title="Refresh"
          >
        ...
        <form onSubmit={handleGo} className="flex-1 min-w-0 h-6">
          <input
            ...
            className="w-full h-full bg-app-panel border border-app-border rounded px-3 text-[11px] text-zinc-400 focus:outline-none focus:border-brand-primary/50 focus:text-zinc-200 transition-all font-mono"
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className="p-1.5 hover:bg-zinc-800/50 rounded-md transition-all text-zinc-500 hover:text-zinc-300"
      ...
      <div ref={containerRef} className="flex-1 relative bg-transparent overflow-hidden">
        ...
        {isLoading && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10 bg-app-bg border border-app-border rounded-full px-3 py-1 flex items-center gap-2 text-xs text-zinc-400">
      ...
      {/* Footer */}
      <div className="h-4 px-2 flex items-center bg-app-bg border-t border-app-border text-[9px] text-zinc-600 font-mono italic justify-between">
