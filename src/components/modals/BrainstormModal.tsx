...
export const BrainstormModal = ({ agents, sessions, onStart, onClose }: Props) => {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-black/85 flex items-center justify-center p-8">
      <div className="w-full max-w-4xl max-h-full border border-zinc-800 bg-[#070709] shadow-xl flex flex-col rounded-lg overflow-hidden relative">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-[#0d0d10]">
          <div>
          ...
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-200 p-2 border border-zinc-800 hover:border-zinc-700 bg-[#0c0c0f] rounded-sm shadow-sm transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] min-h-0 bg-app-bg">
          <form onSubmit={handleSubmit} className="p-6 space-y-6 border-r border-zinc-800">
            <div>
              ...
              <textarea
                ...
                className="mt-2 w-full h-32 bg-[#08080a] border border-zinc-800 focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20 rounded-md text-zinc-200 p-3 text-sm outline-none resize-none transition-all shadow-inner"
            ...
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-3">
                {agents.map(agent => (
                  <label key={agent.id} className="flex items-center gap-3 border border-zinc-800 bg-[#0c0c0f] hover:bg-[#121215] px-3 py-2.5 rounded-md text-sm text-zinc-300 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={participants.includes(agent.name)}
                      onChange={() => toggleAgent(agent.name)}
                      className="accent-brand-accent w-4 h-4 rounded-sm border-zinc-800 bg-[#08080a]"
                    />
            ...
            <div className="flex items-center justify-between gap-4 pt-2">
              <label className="flex items-center gap-3 text-xs font-semibold text-zinc-400 tracking-tight">
                ...
                <input
                  ...
                  className="w-16 bg-[#08080a] border border-zinc-800 rounded-md text-zinc-200 px-3 py-1.5 text-sm outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent/20"
                />
              </label>
              <button
                type="submit"
          ...
          <div className="p-6 min-h-0 flex flex-col bg-[#0b0b0d]">
            ...
            {!activeSession ? (
              ...
              <div className="min-h-0 flex-1 flex flex-col">
                <div className="bg-[#08080a] border border-zinc-800 rounded-t-lg p-3">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-[#0a0a0c]/60 border-x border-b border-zinc-800 rounded-b-lg shadow-inner">
                    ...
                    Object.entries(agentResponses).map(([agentName, responses]) => {
                      ...
                      return (
                        <div 
                          ...
                          className="border border-zinc-800 bg-[#0c0c0f] hover:bg-[#121215] hover:border-zinc-700 p-3 rounded-md cursor-pointer transition-all shadow-sm group"
                        >
        ...
        {selectedAgentId && currentCarouselResponse && (
          <div className="absolute inset-0 z-50 bg-black/95 flex items-center justify-center p-6 animate-in fade-in duration-200">
            <div className="w-full max-w-xl bg-[#0a0a0c] border border-zinc-800 rounded-lg shadow-2xl flex flex-col max-h-full">
