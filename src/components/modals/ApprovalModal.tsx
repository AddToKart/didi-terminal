...
export function ApprovalModal({ agentName, currentProject, onApprove, onReject }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/85 flex items-center justify-center p-6">
      <div className={`w-full ${showDiff ? 'max-w-5xl' : 'max-w-lg'} bg-[#070709] border border-zinc-800 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col rounded-xl transition-all duration-300 max-h-[90vh]`}>
        <div className="px-6 py-5 border-b border-zinc-800 bg-[#0d0d10] flex items-start justify-between gap-4 shrink-0">
          <div>
          ...
          {!showDiff && (
            <button 
              ...
              className="px-3 py-1.5 text-xs font-semibold bg-[#0c0c0f] border border-zinc-800 text-zinc-300 hover:bg-[#121215] hover:text-white transition-colors flex items-center gap-2"
            >
        ...
        <div className={`flex flex-1 min-h-0 ${showDiff ? 'flex-row' : 'flex-col'}`}>
          {showDiff && (
            <div className="flex-1 border-r border-zinc-800 flex flex-col min-w-0 bg-[#040406]">
              <div className="px-4 py-2 border-b border-zinc-800 bg-[#08080a] text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center justify-between">
                Git Diff (HEAD)
          ...
          <div className={`${showDiff ? 'w-80' : 'w-full'} p-6 space-y-4 shrink-0 flex flex-col bg-[#070709]`}>
            <div className="flex-1">
              ...
              <textarea 
                ...
                className="mt-2 w-full h-32 bg-[#08080a] border border-zinc-800 focus:border-brand-accent text-zinc-200 px-3 py-2 text-sm outline-none resize-none rounded-xl shadow-inner"
              />
            </div>
            ...
            <div className={`flex flex-col gap-3 pt-2`}>
              <button 
                ...
                className="w-full px-5 py-2.5 text-xs font-bold bg-[#0c0c0f] border border-zinc-800 text-zinc-300 hover:bg-[#121215] hover:text-white transition-colors flex items-center justify-center gap-2"
              >
